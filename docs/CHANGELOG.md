...
- **Deterministic offline suite**: `grep -q` on a git pipe dies of SIGPIPE under pipefail (use `grep -c`), and the night-orchestrator scenario depended on the host's real battery level. `battery_state` honors a `SOSL_FAKE_PMSET` override; the suite pins it to AC. 46 checks.
