...
- **Self-commit defense**: the agent committed its own work via an allowed tool path — the loop saw a clean tree, logged "No changes", and an unguarded, unmeasured commit rode the branch to the tip (the Judge caught it). HEAD is captured before every Claude call; self-commits are mixed-reset into the working tree so the normal guard → measure → commit pipeline judges them. Scenario s13.
- **Planted-symlink fix**: SOSL symlinks `node_modules`/`.venv` into the worktree for speed, but trailing-slash gitignore patterns match directories only — the scope guard failed on SOSL's own plumbing, `add -A` would commit the link, and every revert's `git clean` deleted it. `SOSL_WT_LINKS` tracks planted links; guards, commits and cleans exclude them. Scenario s12.
- **Exclude-pathspec fix**: real targets gitignore `.sosl/`, and `git add` exits 1 when any pathspec element — `:(exclude)` magic included — names an ignored path. `sosl_add_excludes()` filters the exclude list through `git check-ignore` (an exclude for an ignored path is redundant anyway).
- **Usage-limit handling**: on the Max subscription a mid-turn HTTP 429 (session limit) returns `is_error: true` but `subtype: "success"`, so the first triage logged the useless label "success". Triage now reads `api_error_status` and the result text, labels it `usage_limit`, and the loop stops immediately instead of burning quota to the consecutive-error cap — retrying inside a limited window is futile. Scenario s14.
- **Deterministic offline suite**: `grep -q` on a git pipe dies of SIGPIPE under pipefail (use `grep -c`), and the night-orchestrator scenario depended on the host's real battery level. `battery_state` honors a `SOSL_FAKE_PMSET` override; the suite pins it to AC. 50 checks total.
